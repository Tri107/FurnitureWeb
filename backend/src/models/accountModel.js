...
const AccountModel = {
  ...
  findById: async (id) => {
    const query = 'SELECT account_id, email, is_admin, is_disabled FROM accounts WHERE account_id = ? AND is_disabled = 0';
  ...
  findByRefreshToken: async (token) => {
    const query = 'SELECT * FROM accounts WHERE refresh_token = ? AND is_disabled = 0';
  ...
  update: async (id, data) => {
    ...
    const query = 'UPDATE accounts SET is_admin = ?, is_disabled = ? WHERE account_id = ?';
